...
pub fn BoundedChannel(comptime T: type, comptime capacity: usize) type {
    return struct {
        ...
        pub fn sendWithPrefetch(self: *Self, msg: T) bool {
            if (self.isClosed()) return false;
            return self.queue.pushWithPrefetch(msg);
        }
        ...
        pub fn sendSpin(self: *Self, msg: T, max_spins: u32) bool {
            var spins: u32 = 0;
        ...
        pub fn sendBackoff(self: *Self, msg: T, timeout_ns: u64) bool {
        ...
        pub fn recvTimeoutResult(self: *Self, timeout_ns: u64) RecvResult(T) {
            if (self.recvTimeout(timeout_ns)) |msg| {
                return .{ .message = msg };
            }
...
/// Channel for input messages (I/O → Processor).
pub const InputChannel = BoundedChannel;

/// Channel for output messages (Processor → I/O).
...
test "BoundedChannel - recv result" {
    ...
    switch (r2) {
        .message => |msg| try std.testing.expectEqual(@as(u32, 42), msg),
