...
pub const Processor = struct {
    ...
    fn runLoop(self: *Self) void {
        ...
        while (self.running.load(.acquire)) {
            ...
            if (processed == 0) {
                ...
                // Spin-then-yield-then-sleep pattern for low latency
                if (consecutive_idle > IDLE_SLEEP_THRESHOLD) {
                    // Been idle a long time, brief sleep
    ...
    fn handleMessage(self: *Self, input: *const ProcessorInput) void {
        ...
        // Send outputs - batch push for efficiency
    ...
    /// Called only after initial push failed (slow path).
    ///
    ...
    fn pushOutputWithRetry(self: *Self, output: ProcessorOutput) PushResult {
        ...
        // Still full - yield to let OutputSender thread drain
